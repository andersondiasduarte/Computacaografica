...
void draw() {
  PImage img = loadImage("german_shorthaired_126.jpg"); 
  ...
  for (int y = 0; y < img.height; y++) {
    for (int x = 0; x < img.width; x++) { 
      int pos = (y)*img.width + (x); 
       if(green(imgPB.pixels[pos]) < 200) seg1.pixels[pos] = color(255);
 ...
 for (int y = 0; y < img.height; y++) {
    for (int x = 0; x < img.width; x++) { 
      int pos = (y)*img.width + (x); 
       if(green(imgPB.pixels[pos]) < 150) seg2.pixels[pos] = color(255);
